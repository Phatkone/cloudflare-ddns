...
if ${#zone_id} -le 1 -o ${#fqdn} -le 4 -o ${#api_token} -le 1; then 
    echo "Missing required fields. Please check and try again";
    return 0
fi
...
$ipv4 = $(curl -4 https://${ifconfig}/ip | tr -d '\n' | tr -d '\r')
